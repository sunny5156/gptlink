...
class Order extends Model
{
    ...
    const CHANNEL = [self::CHANNEL_WECHAT => '微信支付', self::CHANNEL_ALIPAY => '支付宝'];
    const STATUS_UNPAID = 1;
    const STATUS_PAID = 2;
    const STATUS = [self::STATUS_UNPAID => '未支付', self::STATUS_PAID => '已支付'];
    const PAY_JSAPI = 'JSAPI';
    const PAY_NATIVE = 'NATIVE';
    const PAY_TYPE = [self::PAY_JSAPI => '微信内网页/小程序', self::PAY_NATIVE => '扫码支付'];
    const PLATFORM_GPT = 1;
    const PLATFORM = [self::PLATFORM_GPT => 'gptlink'];
    /**
     ...
     * @var array
     */
    protected array $fillable = ['trade_no', 'user_id', 'channel', 'platform', 'business_id', 'pay_type', 'payload', 'paid_no', 'price', 'payment', 'status', 'package_id', 'package_name'];
    /**
