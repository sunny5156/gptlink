...
trait MemberPackageTrait
{
    ...
    {
        return MemberPackage::query()->where('user_id', $userId)->where('status', MemberPackage::STATUS_AVAILABLE)->where('type', $type)->exists();
    }
    /**
     * 消费次数
     *
    ...
    {
        logger()->info('更新套餐', ['user_id' => $userId, 'dto' => $dto->toArray()]);
        if ($dto->code) {
            $memberPackage = MemberPackage::query()->where('user_id', $userId)->where('code', $dto->code)->first();
            if ($memberPackage) {
                // 状态如果是无效的，更新为有效状态
