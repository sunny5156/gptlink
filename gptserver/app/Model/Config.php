...
class Config extends Model
{
    ...
    public const TYPE = [self::WECHAT_PLATFORM => '微信公众平台', self::WECHAT_PAYMENT => '微信支付', self::WECHAT_WEB => '微信PC应用配置', self::SMS => '创蓝短信配置', self::WEBSITE => '站点基础配置', self::PROTOCOL => '协议配置', self::PAYMENT => '支付配置', self::KEYWORD => '关键词配置', self::SHARE => '分享配置', self::SALESMAN => '分销配置', self::AI_CHAT => 'ai对话配置', self::AI_IMAGE => 'ai绘画配置', self::LOGIN => '登陆配置'];
    /**
     ...
     * @var array
     */
    protected array $fillable = ['type', 'config'];
